...
window = tk.Tk()
window.title("IoT Based Attendance System using Face Detection")
...
def manually_fill():
    ...
    def fill_attendance():
        ...
        if subb=='':
            ...
            def create_csv():
                ...
                connection.commit()
                csv_name='C:/Users/CW/Documents/New Folder'+DB_table_name+'.csv'
                ...
                with open(csv_name, newline="") as file:
                    ...
                    for col in reader:
                        ...
                        for row in col:
                            ...
                            label = tkinter.Label(root, width=15, height=1, fg="black", font=('times', 13, ' bold '),
            ...
            def attf():
                import subprocess
                subprocess.Popen(r'explorer /select,"C:/Users/CW/Documents/attendance-system-by-face-recognition-in-python-master/Attendance"')
    ...
    fill_manual_attendance = tk.Button(sb, text="Fill Attendance",command=fill_attendance, fg="white", bg="purple", width=20, height=2,
                       activebackground="Red", font=('times', 15, ' bold '))
...
def subjectchoose():
        ...
        if time.time() < future:
            if sub == '':
                ...
                try:
                    ...
                    Notifica.configure(text=e, bg="red", fg="black", width=33, font=('times', 12, 'bold'))
                ...
                col_names = ['Enrolment', 'Name', 'Date', 'Time']
                ...
                while True:
                    ...
                    for (x, y, w, h) in faces:
                        ...
                        if (conf <70):
                            ...
                            date = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d')
                            timeStamp = datetime.datetime.fromtimestamp(ts).strftime('%H:%M:%S')
                            aa = df.loc[df['Enrolment'] == Id]['Name'].values
                            global tt
                            tt = str(Id) + "-" + aa
                            En = '15624031' + str(Id)
                ...
                timeStamp = datetime.datetime.fromtimestamp(ts).strftime('%H:%M:%S')
                Hour, Minute, Second = timeStamp.split(":")
                fileName = "Attendance/"  + "Attendance_"+ sub + date + "_" + ".csv"
                attendance = attendance.drop_duplicates(['Enrolment'])
                print(attendance)
                ...
                ##Create table for Attendance
                date_for_DB = datetime.datetime.fromtimestamp(ts).strftime('%Y_%m_%d')
                DB_Table_name = str( sub + "_" + date_for_DB )
                import pymysql.connections
                ...
                with open(cs) as file:
                    ...
                    for col in reader:
                        ...
                        for row in col:
                            # i've added some styling
                            label = tkinter.Label(root, width=20, height=1, fg="black", font=('times', 12, ' bold '),
                                                  bg="lawn green", text=row, relief=tkinter.RIDGE)
                            label.grid(row=r, column=c)
                            c += 1
                        r += 1
    ...
    def Attf():
        import subprocess
        subprocess.Popen(r'explorer /select,"Attendance"')
    ...
    fill_a = tk.Button(windo, text="Fill Attendance", fg="white",command=Fillattendances, bg="purple", width=20, height=2,
                       activebackground="Red", font=('times', 15, ' bold '))
...
def admin_panel():
    ...
    bg=ImageTk.PhotoImage(Image.open("img/mq.jpg"))
  
    canvas1 = Canvas(window, width = 880,height = 420) 
    canvas1.pack(fill = "both", expand = True,anchor="nw")
            ...
            if password == 'admin123':
                ...
                with open(cs, newline="") as file:
                    ...
                    for col in reader:
                        ...
                        for row in col:
                            # i've added some styling
                            label = tkinter.Label(root, width=15, height=1, fg="black", font=('times', 12, ' bold '),
                                                  bg="lawn green", text=row, relief=tkinter.RIDGE)
                            label.grid(row=r, column=c)
                            c += 1
                        r += 1
...
message = tk.Label(window, text="IoT BASED ATTENDANCE SYSTEM USING FACE DETECTION", bg="cyan", fg="White", width=50,
                   height=3, font=('arial', 20, 'italic bold '))
...
lbl = tk.Label(window, text="Enter Enrollment", width=20, height=1, fg="black", font=('times', 15, ' bold '))
...
txt = tk.Entry(window, validate="key", width=20, fg="black",bg="light grey", font=('times', 20, ' bold '))
txt['validatecommand'] = (txt.register(testVal),'%P','%d')
txt.place(x=400, y=200)

lbl2 = tk.Label(window, text="Enter Name", width=20, fg="black", height=1, font=('times', 15, ' bold '))
lbl2.place(x=150, y=250)

txt2 = tk.Entry(window, width=20, bg="light grey", fg="black", font=('times', 20, ' bold '))
txt2.place(x=400, y=250)

clearButton = tk.Button(window, text="Clear",command=clear,fg="black"    ,width=10  ,height=1 ,activebackground = "Red" ,font=('times', 15, ' bold '))
clearButton.place(x=700, y=200)

clearButton1 = tk.Button(window, text="Clear",command=clear1,fg="black"  ,width=10 ,height=1, activebackground = "Red" ,font=('times', 15, ' bold '))
clearButton1.place(x=700, y=250)

AP = tk.Button(window, text="Check Register students",command=admin_panel,fg="black"  ,bg="light green"  ,width=18 ,height=1, activebackground = "Red" ,font=('times', 15, ' bold '))
AP.place(x=600, y=300)
AP["border"] = "7"


takeImg = tk.Button(window, text="Take Images",command=take_img,fg="black", bg="light green", width=15  ,height=1 ,font=('times', 15, ' bold '))
takeImg["border"] = "7"
takeImg.place(x=150, y=300)

trainImg = tk.Button(window, text="Train Images",fg="black",command=trainimg ,bg="light green"  ,width=15  ,height=1, activebackground = "Red" ,font=('times', 15, ' bold '))
trainImg.place(x=380, y=300)
trainImg["border"] = "7"

FA = tk.Button(window, text="Automatic Attendace",fg="black",command=subjectchoose  ,bg="light green"  ,width=15  ,height=1, activebackground = "Red" ,font=('times', 15, ' bold '))
FA.place(x=150, y=370)
FA["border"] = "7"


quitWindow = tk.Button(window, text="Manually Fill Attendance", command=manually_fill  ,fg="black"  ,bg="light green"  ,width=18  ,height=1, activebackground = "Red" ,font=('times', 15, ' bold '))
quitWindow.place(x=600, y=370)
quitWindow["border"] = "7"
